...
task("soc")
  set_menu {
    ...
    options = {
      ...
      {'c', "clean_verilog", "k", nil, "generate verilog without any debug components"},
      {'d', "dramsim3", "k", nil, "use dramsim3 as simulation main memory"},
      {'e', "enable_perf", "k", nil, "generate verilog without perf debug components"},
  ...
  on_run(function()
    ...
    if not option.get("clean_verilog") and option.get("pldm_verilog") then table.join2(chisel_opts, {"--basic-difftest"}) end
    if not option.get("clean_verilog") and not option.get("pldm_verilog") then table.join2(chisel_opts, {"--enable-difftest"}) end
...
target("emu", function()
  ...
  set_runenv("NOOP_HOME", os.curdir())
