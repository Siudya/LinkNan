...
function _get_sourcefiles_from_cmake(target, targetname, cmakefile)
...
function build_cppfiles(target, batchjobs, sourcebatch, opt)
  ...
  local topname = assert(target:values("verilator.top"), "top module must be assigned!")
  local targetname = "V" .. topname
  local cmakefile = path.join(autogendir, targetname .. ".cmake")

  -- build verilog files
  depend.on_changed(function()
    local argv = { "--cc", "--make", "cmake", "--prefix", targetname, "--Mdir", autogendir, "--top", topname }
    local flags = target:values("verilator.flags")
    if flags then
  ...
  -- get compiled source files
  local sourcefiles = _get_sourcefiles_from_cmake(target, targetname, cmakefile)
  -- do build
  ...
  for _, sourcefile in ipairs(sourcefiles) do
    local objectfile = _objectfile(sourcefile, autogendir)
    local dependfile = _objectfile(objectfile, autogendir)
...
function _objectfile(file, dir)
  local base = path.basename(file)
  local ext = path.extension(file)
  return path.join(dir, base .. ext .. ".o")
end
